...
run_test "A2: two-type-equal-50-50" \
  ...
  "7763,9V74" \
  "yes" \
  "Row1=7763 gates 50%. Good — sibling polling picks the 9V74 winner."
...
run_test "A3: two-type-dominant-fast-90-10" \
  ...
  "7763,9V84" \
  "yes" \
  "CRITICAL: row1 must be the slow type, not the dominant fast type. Gating the fast type (90%) would kill almost all copies. 20% filter must not invert this."
...
run_test "A4: three-type-all-common" \
  ...
  "7763,9V74,9V84" \
  "yes" \
  "Row1=7763 gates 40%. Medium and fast proceed. Good."
...
run_test "A5: three-type-rare-slowest" \
  ...
  "7763,9V74" \
  "yes" \
  "Optimal: filter out 10% very-slow. Gate 7763 (50%) instead — saves 5x more runner-time. Row1=7763."
...
run_test "A6: three-type-rare-fastest" \
  ...
  "7763,9V74" \
  "yes" \
  "Optimal: filter out 10% fast. Gate 7763 (50%). Rare fast type isn't worth tracking."
...
run_test "A7: single-type" \
  ...
  "7763" \
  "yes" \
  "One type. Gating means only copy-a runs. Fine — all runners equal, no benefit from parallelism."

run_test "A8: five-types-all-rare" \
...
run_test "A9: five-types-sub-20" \
  "1 of very-slow, 2 each of slow/medium/fast, 3 very-fast. All <30%." \
  "$(make_profiles 1 "$CPU_VERY_SLOW" 2 "$CPU_SLOW" 2 "$CPU_MEDIUM" 2 "$CPU_FAST" 3 "$CPU_VERY_FAST")" \
  "" \
  "7763,9V74,9V84,9X94" \
  "yes" \
  "Very-slow at 10% filtered. Slow at 20% kept. Row1=7763 gates 20%. 30% fast+very-fast proceed."

run_test "A10: four-type-spread" \
  "4 slow + 3 medium + 2 fast + 1 very-fast (40/30/20/10)." \
  "$(make_profiles 4 "$CPU_SLOW" 3 "$CPU_MEDIUM" 2 "$CPU_FAST" 1 "$CPU_VERY_FAST")" \
  "" \
  "7763,9V74,9V84" \
  "yes" \
  "Very-fast (10%) filtered. Row1=7763 gates 40%."
...
run_test "B2: benchmark-jitter" \
  "Same types, benchmarks fluctuated (9V74 slower this run). Same pool." \
  ...
  "9V74,7763" \
  "no" \
  "Sort order flips in generated file but commit suppressed (pruned=0, added=0). Action keeps using old row1. Benchmark jitter should not cause churn."
...
$(printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' '9V84' '2' '20%' '4500' '650' '4' '15990' 'AMD EPYC 9V84 96-Core Processor')" \
  "7763,9V74,9V84" \
  "no" \
  "9V74 not seen but below a seen top entry → stays. No new types. No commit."
...
run_test "D2: barely-below-threshold" \
  ...
  "7763,9V84" \
  "yes" \
  "CRITICAL: slow filtered but only 1 type remains → fallback keeps all. Row1 must be slow type."
...
run_test "D3: three-types-one-dominates" \
  ...
  "7551,9V74,9V84" \
  "yes" \
  "Only fast (80%) passes 20% filter → 1 type → fallback to all. Row1=very-slow."
...
run_test "D4: two-rare-one-common" \
  ...
  "7551,7763,9V84" \
  "yes" \
  "Only fast passes 20% → 1 type → fallback to all. Row1=7551. All three in TSV."
...
run_test "E1: four-type-gradual" \
  ...
  "7551,7763,9V74,9V84" \
  "yes" \
  "All >= 20%. Row1=7551 gates 30%. But: 7763 (30%) also slow — single pattern can only gate one. ACTION LIMITATION: ideally gate both 7551+7763 (60% total slow runners)."

run_test "E2: two-slow-one-fast" \
  "4 very-slow + 4 slow + 2 fast (40/40/20). Two equally common slow types." \
  "$(make_profiles 4 "$CPU_VERY_SLOW" 4 "$CPU_SLOW" 2 "$CPU_FAST")" \
  "" \
  "7551,7763,9V84" \
  "yes" \
  "Row1=7551 gates 40%. But 7763 (40%) is also slow and proceeds ungated. ACTION LIMITATION: single pattern misses 40% of slow runners."
...
run_test "E3: staircase-five-types" \
  ...
  "7551,7763,9V74,9V84" \
  "yes" \
  "Very-fast (10%) filtered. Row1=7551 gates 30%. ACTION LIMITATION: 7763 at 20% also slow."
...
run_test "E4: many-types-no-majority" \
  ...
  "7551,7763,9V74,9V84,9X94" \
  "yes" \
  "All at 20%. Row1=7551 gates 20%. 80% proceed. Sibling polling picks fastest."
...
run_test "F1: ranking-would-help" \
  ...
  "7551,7763,9V84" \
  "yes" \
  "Row1=7551 gates 30%. 7763 (30% slow) proceeds and wastes time. WITH RANKING: runners could read their TSV position and defer to higher-ranked (faster) siblings. Would gate 60% of slow runners total."
